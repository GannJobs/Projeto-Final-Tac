...
function Enemy01:new()
    ...
    aniDogStop = anim.newAnimation(grid('1-1', 1), 0.1)
    ...
    aniDogRun = anim.newAnimation(grid('1-3', 1, '1-3', 2, '1-2', 3), 0.1)
...
function Enemy01:draw()
    if self.movimento == true then
            aniDogRun:draw(self.andando, self.posicao.x, self.posicao.y, 0, direcaoDog, 1, 32, 0)
    else
            love.graphics.draw(self.parada, self.posicao.x, self.posicao.y, 0, direcaoDog, 1, 32, 0)
    end
end
