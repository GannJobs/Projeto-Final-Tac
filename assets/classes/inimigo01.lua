...
function Enemy01:draw()
    ...
    for i = 1, 15 do
        if self.inimigos01[i].Hp > 0 then
            ...
            love.graphics.rectangle("fill", self.inimigos01[i].posicao.x - self.inimigos01[i].Hp / 2,
                self.inimigos01[i].posicao.y - 42, self.inimigos01[i].Hp, 6)
            love.graphics.setColor(1, 1, 1)

            -- animaçoes

            if self.inimigos01[i].movimento == true and self.inimigos01[i].atacando == false then
                self.inimigos01[i].aniDogRun:draw(self.andando, self.inimigos01[i].posicao.x,
                    self.inimigos01[i].posicao.y - 32, 0, self.inimigos01[i].direcaoDog * 2, 2, 32, 16)
            else
                self.inimigos01[i].aniDogStop:draw(self.parada, self.inimigos01[i].posicao.x,
                    self.inimigos01[i].posicao.y - 32, 0, self.inimigos01[i].direcaoDog * 2, 2, 32, 16)
            end

            if self.inimigos01[i].atacando == true then
                self.inimigos01[i].aniMordida:draw(self.mordida, self.inimigos01[i].posicao.x,
                    self.inimigos01[i].posicao.y - 32, 0, self.inimigos01[i].direcaoDog * 2, 2, 32, 16)
            end
        end
    end
...
function Enemy01:Attack(dt)
    ...
    for i = 1, 15 do
        if RangeAttack(hero.Contato, self.inimigos01[i].Contato, hero.posicao, self.inimigos01[i].posicao) and
            self.inimigos01[i].atacando == false then
...
function Enemy01:move(dt)
    ...
    for i = 1, 15 do
        if RangeVisao(self.inimigos01[i].raioDeteccao, self.inimigos01[i].posicao, hero.posicao) then
            ...
            self.inimigos01[i].vel_desejada = (hero.posicao - self.inimigos01[i].posicao) / 12
            self.inimigos01[i].forca_direcao = self.inimigos01[i].vel_desejada - self.inimigos01[i].velocidade
            self.inimigos01[i].velocidade = self.inimigos01[i].velocidade + self.inimigos01[i].forca_direcao /
                                                self.inimigos01[i].massa
            ...
            if self.inimigos01[i].posicao.x > self.inimigos01[i].origem.x then
                self.inimigos01[i].direcao = 1
                self.inimigos01[i].direcaoDog = self.inimigos01[i].direcao
            else
                self.inimigos01[i].direcao = -1
            ...
            self.inimigos01[i].vel_desejada = (self.inimigos01[i].origem - self.inimigos01[i].posicao) / 9
